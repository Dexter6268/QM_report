general_overview = """质量背景分析。按照顺序进行分析。
分析的角度包括但不限于出台的质量政策，开展的质量相关活动，经济总体情况，产业结构等
1.结合地区数据的内容，以数值作为参考概述该地区总体质量发展情况。
2.结合提供的数据，参考近年来该地支持制造业高质量发展出台的相关政策文件及内容，以及支持制造业高质量发展开展的相关举措，如开展的产业链质量提升活动、促销费活动等，分析该地质量工作在哪些类别或者哪些地区取得了成效，或是需要加强（如有）。
3.结合提供的数据按以下顺序进行分析，
   (1) 阐述该地区产品质量合格率数值在全国排名情况和与往年相比排名变动情况。
   (2) 分析其合格率和排名的变动趋势与该地区开展的制造业质量提升相关举措之间可能存在的关系。
   (3) 对比与本地区数值处于相同梯队的其他地区变化情况，分析本地区数值变动是领先还是落后于相同梯队的其他地区。
"""

regional_overview = """仅依据提供的数值进行分析，不要结构化列出，以段落格式输出.
1.首先先对总体情况进行简短的概括，要求点明数值。
2.然后把各个地区对应值按照由高到低的顺序进行聚类分析并分别进行简要评价。数值相近的类别共同评价但要求所有地区都要按照如下格式指明数值和排名。例如：（长沙 95.58%，排名第1）
"""

industry_overview = """仅依据提供的数值进行分析，不要结构化列出，以段落格式输出
1.然后把各个类别对应值按照由高到低的顺序进行聚类分析并分别进行简要评价。数值相近的类别共同评价但要求所有类别都要按照如下格式指明数值。例如：（食品加工98.21%），
2. 寻找类别营业收入规模最高或者最低的类别与大类数据的高低是否有特殊关系，分析这些大类产品质量水平与各大类营业收入规模的关系。
"""

industry_detail = """概述该地区经济发展情况，重点包括制造业发展情况。
在支持制造业高质量发展出台的相关政策文件及内容，以及支持制造业高质量发展开展的相关举措，如开展的产业链质量提升活动、促销费活动等。
阐述该地区近年来产品质量合格率数值以及在全省排名情况，简要分析其合格率情况与该地区开展的制造业质量提升相关举措之间可能存在的关系。
"""

regional_detail = """结合本地区的产业结构的实际情况与本行业的特点进行分析
1.该行业发展概况，包括不限于各类经济指标、重点产品和重点企业情况。
2.阐述该行业产品质量合格率情况。
3.阐述该行业抽检产品不合格情况，包括不合格较多的产品及其不合格原因。
"""
...
- 第四章的问题分析要求如下
   - 必须对应于第二章和第三章的分析
   - 分析重点产业产品质量合格率情况及变动趋势，判断重点产业产品质量合格率对制造业整体质量合格率带动作用强弱。如重点行业产品质量合格率明显低于或接近地区平均水平，则其带动作用不强，简要分析其原因。形成观点作为标题。
   - 分析下辖地区产品质量合格率波动情况，各地区产品质量合格率差异是否显著。若各地区产品质量合格率差异较大，则地区产品质量发展不协调，从经济发展、质量意识等方面简要分析其原因，形成观点作为标题。
   - 分析不合格较多的产品情况，判断不合格产品所属行业是传统行业或新兴行业等，从其所属行业发展情况分析其产业发展存在的问题，形成观点作为标题。
   - 分析不合格产品生产企业情况，根据不合格产品生产企业属于大中小微型企业的情况分析该地区各类企业在质量管理上可能存在的问题，形成观点作为标题。
- 第五章的政策建议必须对应于第四章的问题分析，根据文章内容及主要问题写3到4条政策建议。政策建议可以结合《质量强国建设纲要》等相关质量政策文件提出。
- 重点：每个节需包含以下字段：
   - name - 该节标题
...
"""
compile_final_report_instructions = """你是一个报告润色专家，负责按照如下要求整合和润色报告的最终内容。
1. 在保持报告其它内容不变的前提下，在报告最后单设一级标题：参考文献
2. 将各节后的参考文献全部挪到参考文献标题下
3. 合并相同的参考文献，并按文中引用的顺序从[1]开始重新编号
4. 文中的参考文献引用也相应地从[1]开始重新编号
5. 检查各级标题是否有重复现象（比如第3章 第三章这种），如果有的话把重复标题的去掉
6. 重点：输出调整后的完整报告内容
以下是报告的完整内容，请按照上述要求进行整合和润色：
{report_content}
"""
