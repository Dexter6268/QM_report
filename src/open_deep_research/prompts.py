...
- research - 是否需进行联网搜索（关键要求：主体章节[除了最后两章]必须设置Research=True。有效报告至少需包含2-3个需调研章节）
- content - 章节具体内容（暂留空）
整合准则：
...
- 避免包含与核心主题间接相关的边缘内容
- 重点：若报告范围为全国则地区列举所有省份，若报告范围为某省份则地区列举所有地级市；若报告主题指定为<测试报告>则只列举两三个代表省份或地级市
...
section_writer_instructions = """撰写报告的一个节
